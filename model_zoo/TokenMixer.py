...
from fuxictr.pytorch.layers import FeatureEmbedding, MLP_Block, Dice, DIN_Attention, PerTokenSwiGLU, MultiHeadTokenMixing
...
class TokenMixer(MultiTaskModel):
    def __init__(self,
        ...
        self.unified_layers = TokenMixerBlocks(input_dim=token_dim,
                                         num_tokens=num_global_token + num_field,
    ...
    def forward(self, inputs):
        ...
        pooling_emb = self.attention_layers(target_emb, sequence_emb, mask) # B x item_info_dim

        # 其它非序列特征 -> NS tokens
        batch_emb = self.embedding_layer(batch_dict, flatten_emb=False)       # B x F × embedding_dim
        context_tokens = torch.cat([batch_emb, target_emb.view(batch_size, -1, self.embedding_dim),
                                    pooling_emb.view(batch_size, -1, self.embedding_dim)], dim=1)
        context_tokens = self.tokenizer_layer(context_tokens)                 # B x (F + seq + target) × embedding_dim
...
class Embedding2Tokenization(nn.Module):
    def __init__(self, embedding_dim, token_dim, num_global_token, num_field):
    ...
    def forward(self, feature_embeddings): #
        """
        feature_embeddings: B x (F + seq + target) × embedding_dim
        return: B x (num_global_token + F + seq + target) x token_dim
        """
        global_tokens = self.mlp(feature_embeddings.flatten(start_dim=1))
        global_tokens = global_tokens.view(-1, self.num_global_token, self.token_dim)
        ns_tokens = torch.einsum("btd,tdk->btk", feature_embeddings, self.W)
        ns_tokens = torch.cat([global_tokens, ns_tokens], dim=1)
        return ns_tokens
...
class TokenMixerBlocks(nn.Module):
    def __init__(self,
        ...
        self.mixer_layers = nn.ModuleList([
            MultiHeadTokenMixing(input_dim=input_dim, num_token=num_tokens)
            for _ in range(num_layers)
        ])
    ...
    def forward(self, x: torch.Tensor): # B x T x token_dim
        for i in range(self.num_layers):
            ...
            mix_x = self.pffn_layers1[i](mix_x) + mix_x
