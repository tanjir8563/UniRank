...
class TokenFormer(MultiTaskModel):
    ...
    def forward(self, inputs):
        ...
        target_emb = item_feat_emb[:, -1, :].unsqueeze(1)   # B x 1 x item_info_dim
        ...
        unified_tokens, unified_valid_mask = self.unified_tokenizer_layer(
            user_profile, sequence_emb, target_emb, seq_mask=mask
        )

        # unified model
        unified_tokens = self.unified_layers(unified_tokens, valid_mask=unified_valid_mask)

        tower_output = [self.tower[i](unified_tokens[:, -1, :]) for i in range(self.num_tasks)]
...
class Embedding2Tokenization(nn.Module):
    def __init__(self, embedding_dim, item_info_dim, token_dim, user_feature_num):
        ...
        self.W_u = nn.init.xavier_uniform_(nn.Parameter(torch.empty(user_feature_num, embedding_dim, token_dim)))
        ...
        self.SEP1 = nn.init.xavier_uniform_(nn.Parameter(torch.empty(1, 1, token_dim)))
        self.SEP2 = nn.init.xavier_uniform_(nn.Parameter(torch.empty(1, 1, token_dim)))
    ...
    def forward(self, user_profile, sequence_emb, target_emb, seq_mask=None):
        ...
        seq_mask:     B x T, 1 for valid seq positions
        ...
        unified_tokens = torch.cat([user_tokens, SEP1, seq_tokens, SEP2, target_tokens], dim=1)
...
class UnifiedInteractionBlocks(nn.Module):
    def __init__(self,
        ...
        self.ffn_layers = nn.ModuleList([
            SwiGLU(input_dim=token_dim, expand=expand, net_dropout=net_dropout)
            for _ in range(self.num_layers)
        ])
...
class GatedMultiHeadAttentionLayer(nn.Module):
    ...
    @staticmethod
    def _build_block_mask(S, device, num_head, user_feature_num, attn_mode="full", window_size=None):
        """
        unified stream:
    ...
    def forward(self, x: torch.Tensor, valid_mask=None, attn_mode="full", window_size=None):  # B x S x D
        B, S, D = x.shape
        if valid_mask is not None:
            x = x * valid_mask.unsqueeze(-1).to(dtype=x.dtype)
        Q = self.W_q(x).view(B, S, self.num_head, self.head_dim).transpose(1, 2)  # [B, H, S, Dh]
        ...
        score_mod = None
        if valid_mask is not None:
            valid_key_mask = valid_mask.bool()

            def score_mod(score, b, h, q_idx, kv_idx):
                return torch.where(valid_key_mask[b, kv_idx], score, -float("inf"))
